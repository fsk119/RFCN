...
def sample_fast_rcnn_targets(boxes, gt_boxes, gt_labels):
    ...
    def sample_fg_bg(iou):
        ...
        num_total_fg = tf.size(fg_inds, name='num_total_fg')
        ...
        add_moving_summary(num_fg, num_bg, num_total_fg)
...
def fastrcnn_outputs(feature, num_classes, useDropout=False):
    """
    Args:
        feature (any shape):
    ...
    """
    if useDropout:
        feature = Dropout('finalDropout', feature, rate=0.5)
    classification = FullyConnected(
        'transferClass', feature, num_classes,
        kernel_initializer=tf.random_normal_initializer(stddev=0.01))
    box_regression = FullyConnected(
        'transferBox', feature, num_classes * 4,
...
@under_name_scope
def rfcn_predicitions(boxes, probs):
    """
    :param boxes: n x 4
    :param probs: probs
    :return:
...
@under_name_scope()
def rfcn_losses(labels, label_logits, fg_boxes, fg_box_logits):
    """
    Args:
        labels: n,
        label_logits: nxC
    ...
    fg_inds = tf.where(labels > 0)[:, 0]
    num_fg = tf.size(fg_inds)
    fg_box_logits = tf.gather(fg_box_logits, fg_inds)
