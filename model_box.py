...
def VotePooling(featuremap, boxes, k, poolshape, isCls=True):
    """
    1. split boxes into k^2 region
    2. calculate value of every region. we assume structure of featuremap is [k^2, numClass]
    ...
    scoremap_bins = []
    base = 1 if isCls else 4
    print(ret.shape)
    for k_step in range(k*k):
        offset_x = int((k_step//k)*poolshape)
        offset_y = int((k_step%k)*poolshape)
        channel_indices = np.arange(k_step*config.DATA.NUM_CLASS*base, (k_step+1)*config.DATA.NUM_CLASS*base,
                                    dtype=np.int32)
        # shape B x k*poolshape x k*poolshape x k*k*C*base-> B x poolshape x poolshape x C*base
        cropRegion = tf.image.crop_to_bounding_box(tf.gather(ret, channel_indices, axis=-1), offset_y, offset_x, poolshape, poolshape, )
        # B x poolshape x poolshape x C*base-> B x 1 x 1 x C*base->0
        cropmean = tf.nn.avg_pool(cropRegion, (1, poolshape, poolshape, 1),
                                    (1, poolshape, poolshape, 1), padding='VALID', data_format='NHWC')
        # print(cropmean.shape)
        cropmean = tf.transpose(cropmean, [0, 3, 1, 2])
        cropmean = tf.squeeze(cropmean, axis=-1)
        scoremap_bins.append(cropmean)
    scoremap_bins = tf.concat(scoremap_bins, axis=-1)
    return tf.reduce_sum(scoremap_bins, axis=-1)/(k*k)
