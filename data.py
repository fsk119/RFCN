...
def get_train_dataflow():
    ...
    with open(cfg.DATA.TRAIN, 'rb') as f:
        imgs = pickle.load(f)
    """
    ...
    def preprocess(img):
        fname, boxes, klass, is_crowd = img['file_name'], img['boxes'].astype(np.float32), img['class'], img['is_crowd']
        boxes = np.copy(boxes)
...
def get_eval_dataflow(shard=0, num_shards=1):
    ...
    with open(cfg.DATA.VAL, 'rb') as f:
        imgs = pickle.load(f)
    num_imgs = len(imgs)
