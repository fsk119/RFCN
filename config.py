...
_C.DATA.TRAIN = './train.pickle'#['train2014', 'valminusminival2014']   # i.e., trainval35k
_C.DATA.VAL = './validation.pickle'#'minival2014'   # For now, only support evaluation on single dataset
_C.DATA.EVAL = './test.pickle'
_C.DATA.NUM_CATEGORY = 1#80    # 80 categories.
_C.DATA.CLASS_NAMES = ['BG', 'abnormal']  # NUM_CLASS (NUM_CATEGORY+1) strings, to be populated later by data loader. The first is BG.
...
_C.TRAIN.NUM_EVALS = 100  # number of evaluations to run during training

# preprocessing --------------------
# Alternative old (worse & faster) setting: 600, 1024
_C.PREPROC.SHORT_EDGE_SIZE = 1024
...
# rpn training -------------------------
_C.RPN.FG_RATIO = 0.8  # fg ratio among selected RPN anchors
_C.RPN.BATCH_PER_IM = 196  # total (across FPN levels) number of anchors that are marked valid
_C.RPN.MIN_SIZE = 0
...
_C.RPN.TRAIN_PER_LEVEL_NMS_TOPK = 1000
...
_C.FRCNN.FG_RATIO = 0.8  # fg ratio in a ROI batch
...
_C.TEST.RESULT_SCORE_THRESH = 0.6
