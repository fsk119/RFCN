...
def backbone_scope(freeze):
    ...
    with argscope([Conv2D, MaxPooling, BatchNorm], data_format='channels_first'), \
        ...
        if freeze:
            stack.enter_context(freeze_variables(stop_gradient=True, skip_collection=True))
        else:
...
def resnet_bottleneck(l, ch_out, stride, useDropout=False):
    shortcut = l
    ...
    if useDropout:
        ret = Dropout('dropout', ret, rate=0.5)
    return tf.nn.relu(ret, name='output')


def resnet_group(name, l, block_func, features, count, stride, useDropout=False):
    with tf.variable_scope(name):
        for i in range(0, count):
            with tf.variable_scope('block{}'.format(i)):
                l = block_func(l, features, stride if i == 0 else 1, useDropout)
    return l
...
def resnet_conv5(image, num_block, useDropout=False):
    with backbone_scope(freeze=False):
        l = resnet_group('group3', image, resnet_bottleneck, 512, num_block, 2, useDropout)
        return l
...
def GroupNorm2(x, group, gamma_initializer=tf.constant_initializer(1.)):
    """
...
def vgg16_fc(image):
    with argscope(Conv2D, kernel_initializer=tf.variance_scaling_initializer(scale=2.)), \
         argscope([Conv2D, MaxPooling, BatchNorm], data_format='channels_first'):
        x = FullyConnected('fc6', image, 4096, kernel_initializer=tf.random_normal_initializer(stddev=0.001), activation=tf.nn.relu)
        x = Dropout('drop0', x, rate=0.5)
        x = FullyConnected('fc7', x, 4096, kernel_initializer=tf.random_normal_initializer(stddev=0.001), activation=tf.nn.relu)
